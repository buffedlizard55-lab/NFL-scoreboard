...
(function (root, factory) {
  ...
  function scoringPlayInfo(plays, index) {
    ...
    return {
      index: index,
      id: plays[index].id,
      type: playTypeText(plays[index]),
      text: playText(plays[index]),
  ...
  function nearestScoringPlay(plays, index) {
    ...
    for (let i = index - 1; i >= start; i -= 1) {
      ...
      if (!p) continue;
      if (isScoringPlay(p)) {
        const info = scoringPlayInfo(plays, i);
        return {
          id: info.id,
          type: info.type,
          text: info.text,
  ...
  /* ------------------- score-removal risk ("may remove points") ---------- */
  /*
   * Points can be taken off the board before ESPN publishes the corrected
   * running score: a flag is down on a play that was ruled a score, or a
   * touchdown/field goal is "under review". While that situation is live the
   * running score still shows the points, so a drop-only check cannot flag it.
   *
   * To keep this honest (no invented corrections), the helpers below only
   * label an event as at-risk when it is anchored to a real scoring play:
   *
   *   - the event play itself was the scoring play and was updated in place
   *     with review wording (scoringPlay stays true on the same record), or
   *   - the scoring play is the nearest record behind a contiguous chain of
   *     booth records (score -> "Play under review." -> replay verdict, or
   *     score -> the nullifying penalty record).
   *
   * Ordinary plays break the chain, so a mid-drive review or an unrelated
   * flag several plays after a score is never labeled. A measured drop in the
   * running score remains authoritative: that state is "removed", not merely
   * "possible". Once a verdict is published the chain resolves: overturned ->
   * "removed", confirmed/stands/declined -> "stood" (points stay).
   */
  const BOOTH_SCORE_CHAIN_MAX = 4; // booth records allowed between score and event

  function linkedScoringPlay(plays, index) {
    if (!plays || index == null || index < 0 || index >= plays.length) return null;
    const self = plays[index];
    if (self && self.scoringPlay === true && classifyBooth(self)) {
      return { index: index };
    }
    let i = index - 1;
    let distance = 1;
    while (i >= 0 && distance <= BOOTH_SCORE_CHAIN_MAX) {
      const p = plays[i];
      if (!p) return null;
      if (isScoringPlay(p)) return { index: i };
      if (!classifyBooth(p)) return null; // an ordinary play ends the chain
      i -= 1;
      distance += 1;
    }
    return null;
  ...
  /*
   * The next verdict in the same review chain, so a "Play under review." that
   * later resolves to "Upheld." is not left labeled at-risk forever.
   */
  function boothResolutionAhead(plays, index) {
    const maxIndex = Math.min(plays.length, index + BOOTH_SCORE_LOOKAHEAD + 1);
    for (let j = index + 1; j < maxIndex; j += 1) {
      const p = plays[j];
      if (!p || !classifyBooth(p)) return ''; // ordinary play ends the chain
      const result = boothResult(playText(p));
      if (result && result !== 'pending') return result;
    }
    return '';
  }

  function boothScoreRisk(plays, index, event, effect) {
    const empty = {
      risk: 'none',
      reason: '',
      scoringPlay: null,
    ...
    if (!plays || index == null || index < 0 || index >= plays.length) return empty;

    const eff = effect || boothScoreEffect(plays, index);
    const ev = event || {};
    const link = linkedScoringPlay(plays, index);
    const scoringPlay = link ? scoringPlayInfo(plays, link.index) : null;
    const text = ev.text || playText(plays[index]);
    const result = ev.result || boothResult(text);
    const kind = ev.kind || classifyBooth(plays[index]);
    const out = {
      risk: 'none',
      reason: '',
      scoringPlay: scoringPlay,
      removesPoints: eff.removesPoints,
      pointsRemoved: eff.pointsRemoved,
      team: eff.team
    };

    // A measured drop in ESPN's running score is authoritative: points really
    // were taken off, whatever the wording says.
    if (eff.removesPoints) {
      return Object.assign(out, {
        risk: 'removed',
        reason: "ESPN's running score dropped — points were removed from the board."
      });
    }
    if (!link) return out;
    ...
    if (result === 'declined') {
      return Object.assign(out, {
        risk: 'stood',
        reason: 'The penalty was declined — the points stand.'
      });
    }
    if (result === 'confirmed' || result === 'stands') {
      return Object.assign(out, {
        risk: 'stood',
        reason: 'The scoring play was confirmed — the points stand.'
      });
    }
    if (result === 'overturned') {
      return Object.assign(out, {
        risk: 'removed',
        reason: 'The scoring play was reversed — points are off the board.'
      });
    }
    if (result === 'offsetting') {
      return Object.assign(out, {
        risk: 'possible',
        reason: 'Offsetting penalties replay the scoring play — points may be taken off.'
      });
    }
    if (result === 'pending' || !result) {
      const ahead = boothResolutionAhead(plays, index);
      if (ahead === 'confirmed' || ahead === 'stands' || ahead === 'declined') {
        return Object.assign(out, {
          risk: 'stood',
          reason: 'The review ended — the points stand.'
        });
      }
      if (ahead === 'overturned') {
        return Object.assign(out, {
          risk: 'removed',
          reason: 'The review ended in a reversal — points are off the board.'
        });
      }
    }

    if (kind === 'penalty' && !/no play/i.test(text) &&
        /\bunsportsmanlike\b|\bunsportsman-like\b|\bdisqualif/i.test(text)) {
      // Unsportsmanlike conduct is a dead-ball foul; it is enforced on the try
      // or kickoff, so it cannot nullify a score that is already on the board.
      return Object.assign(out, {
        risk: 'stood',
        reason: 'Unsportsmanlike conduct is a dead-ball foul — the points stand.'
      });
    }
    if (kind === 'penalty') {
      return Object.assign(out, {
        risk: 'possible',
        reason: /no play/i.test(text)
          ? 'A penalty nullified the play — points are coming off the board.'
          : 'A penalty was called on a scoring play — points may be taken off.'
      });
    }
    if (kind === 'review') {
      return Object.assign(out, {
        risk: 'possible',
        reason: 'The scoring play is under review — points may be taken off.'
      });
    }
    if (kind === 'challenge') {
      return Object.assign(out, {
        risk: 'possible',
        reason: 'The scoring play was challenged — points may be taken off.'
      });
    }
    if (kind === 'replay') {
      return Object.assign(out, {
        risk: 'possible',
        reason: 'A replay review is checking the scoring play — points may be taken off.'
      });
    }
    return Object.assign(out, {
      risk: 'possible',
      reason: 'The event is tied to a scoring play — points may be taken off.'
    });
  }
  ...
  function boothEventContext(event, plays, index) {
    ...
    const risk = boothScoreRisk(plays, index, event, effect);
    const related = risk.scoringPlay || nearestScoringPlay(plays, index);
    return Object.assign({}, event, {
      ...
      relatedScoringPlay: related ? {
        index: related.index != null ? related.index : null,
        id: related.id,
        type: related.type,
        text: related.text,
  ...
  return {
    ...
    boothScoreRisk: boothScoreRisk,
    linkedScoringPlay: linkedScoringPlay,
