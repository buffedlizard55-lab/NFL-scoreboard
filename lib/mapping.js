...
(function (root, factory) {
  ...
  function isTouchdownBoundaryChallengeText(text) {
    const t = String(text || '');
    if (!t) return false;
    // Breaking boundary / plane / goal line / pylon / end zone
    if (/\b(?:broke|broken|break|breaks|cross|crossed|crossing|reach|reached|reaching|touch|touched|touches)\s+(?:the\s+)?(?:plane|boundary|goal\s*line|pylon|end\s*zone)\b/i.test(t)) return true;
    // Challenged / review ... for a touchdown
    if (/\b(?:challenged?|reviews?|under\s+review|replays?)\b[\s\S]*\b(?:for\s+a|as\s+a|if\s+a|whether\s+a|for|whether)\s+touchdown\b/i.test(t)) return true;
    if (/\b(?:for\s+a|as\s+a|if\s+a|whether\s+a|for|whether)\s+touchdown\b[\s\S]*\b(?:challenged?|reviews?|under\s+review|replays?)\b/i.test(t)) return true;
    // Short of goal line / down at the 1 / out of bounds at 1
    if (/\b(?:short\s+of\s+(?:the\s+)?goal\s*line|down\s+at\s+(?:the\s+)?[1-9]|out\s+of\s+bounds\s+(?:at|near)\s+(?:the\s+)?[1-9]|in\s+the\s+end\s*zone)\b[\s\S]*\b(?:challenged?|reviews?|under\s+review|replays?)\b/i.test(t)) return true;
    if (/\b(?:challenged?|reviews?|under\s+review|replays?)\b[\s\S]*\b(?:short\s+of\s+(?:the\s+)?goal\s*line|down\s+at\s+(?:the\s+)?[1-9]|out\s+of\s+bounds\s+(?:at|near)\s+(?:the\s+)?[1-9]|in\s+the\s+end\s*zone)\b/i.test(t)) return true;
    // General challenge/review with touchdown keyword
    if (/\b(?:challenge|challenged|replay|review|under review)\b/i.test(t) && /\btouchdown\b/i.test(t)) return true;
    return false;
  }
  ...
  function scoreRulingDetails(event, plays, index, effect) {
    ...
    const isRzTdChallenge = !!(event && event.redZone &&
      (event.kind === 'challenge' || event.kind === 'review' || event.kind === 'replay') &&
      isTouchdownBoundaryChallengeText(event.text));

    const canWatch = !!((scoringPlay && event &&
      (event.kind === 'penalty' || event.kind === 'review' || event.kind === 'challenge' || event.kind === 'replay') &&
      !isKnownPostScoreEnforcement(play)) || isRzTdChallenge);

    const isOverturnedRzTd = isRzTdChallenge && event && event.result === 'overturned';
    const sourceResolved = canWatch && !nullified && !isOverturnedRzTd &&
      (finalNonNullifiedResult(event && event.result) || sourceMovedPastScoringRuling(plays, index));
    const pending = !nullified && !isOverturnedRzTd && canWatch && !sourceResolved;

    let scoringWatch = '';
    if (nullified) scoringWatch = 'nullified';
    else if (isOverturnedRzTd) scoringWatch = 'awarded';
    else if (pending) scoringWatch = 'pending';
    else if (sourceResolved) scoringWatch = 'retained';
    ...
    else if (isOverturnedRzTd) evidence = 'overturned touchdown boundary challenge';
    else if (isRzTdChallenge && pending) evidence = 'red zone touchdown boundary challenge pending';
    else if (isRzTdChallenge && sourceResolved) evidence = 'touchdown boundary challenge upheld / stands';
  ...
  function boothEventContext(event, plays, index) {
    ...
    const withScores = Object.assign({}, event, {
      ...
      isRedZoneTouchdownChallenge: ruling.isRedZoneTouchdownChallenge,
      contextIndex: index
    });
  ...
  function boothEvent(p, drive) {
    ...
    isRedZoneTouchdownChallenge: isRedZonePlay(p, drive) &&
      (kind === 'challenge' || kind === 'review' || kind === 'replay') &&
      isTouchdownBoundaryChallengeText(text),
  ...
  /*
   * An alertable scoring event is any event that directly affects the scoring
   * outcome: confirmed nullifications, pending reviews/challenges/penalties on
   * scoring plays, red-zone touchdown boundary challenges, or awarded scores.
   */
  function isScoringAlertEvent(event) {
    if (!event || event.irregularity || event.kind === 'integrity' || event.scoringWatch === 'irregular') return false;
    if (isConfirmedNullifiedScoringEvent(event)) return true;
    if (event.scoringRuling && (event.scoringWatch === 'pending' || event.scoringWatch === 'awarded')) return true;
    if (event.redZone && event.isRedZoneTouchdownChallenge &&
        (event.scoringWatch === 'pending' || event.scoringWatch === 'awarded')) return true;
    return false;
  }
  ...
  return {
    ...
    scoreKindFromText: scoreKindFromText,
    ...
    isScoringAlertEvent: isScoringAlertEvent,
    isTouchdownBoundaryChallengeText: isTouchdownBoundaryChallengeText,
