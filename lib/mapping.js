...
(function (root, factory) {
  ...
  /*
   * A falling per-play score is useful only when the booth event can actually
   * be the ruling that removed it. ESPN's live feed can briefly publish a
   * stale 0-0 score on the kickoff after a touchdown. A return-team penalty on
   * that kickoff is not a ruling on the touchdown, so treating the temporary
   * 7 -> 0 as a rollback produces a false nullification.
   *
   * Keep score-drop evidence for:
   *   - a scoring/nullification play itself;
   *   - a review, challenge, or replay ruling; and
   *   - a standalone penalty entry immediately after the scoring play (with
   *     only an official/regular timeout allowed between them).
   * A routine kickoff/punt/return foul is a new play and cannot remove the
   * preceding score unless its own text explicitly says a score was nullified.
   */
  function scoreRollbackCanBelongToEvent(plays, index) {
    const play = plays && plays[index];
    if (!play) return false;
    const text = playText(play);
    if (play.scoringPlay === true || mentionsScore(text)) return true;
    ...
    // A separately published penalty correction belongs to the score only
    // when that score is the preceding substantive play. Do not reach back
    // across a kickoff, scrimmage play, or other change of possession.
    for (let i = index - 1; i >= 0 && i >= index - 3; i -= 1) {
      const prior = plays[i];
      if (!prior) continue;
      const priorType = playTypeText(prior);
      if (/timeout/i.test(priorType)) continue;
      return isScoringPlay(prior);
    }
    return false;
  }
  ...
  function boothScoreEffect(plays, index) {
    ...
    // Carrying the last valid total is safer than displaying ESPN's temporary
    // 0-0 as if officials removed points.
    const during = publishedDropsScore && !rollbackEligible
      ? before
      : publishedDuring;
    ...
    // but only for an event that could have caused that score correction.
    ...
    if (resolved) {
      ...
      // No rollback was observed. Use the next published score only when it is
      // nondecreasing; another stale lower total is not a valid post-event
      // state for an unrelated flag.
  ...
  /* ------------------- nullified scores (booth "removals") --------------- */
  /*
   * The booth feed's Red Zone tab, its "Nullified" filter and the alert sound
   * all track ONE thing: a score that was taken off the board, or a play that
   * ESPN's play-by-play text explicitly reports as a nullified score.
   *
   * Two independent, verified signals are used. Neither is guessed:
   *
   *   1. removesPoints — ESPN's own running score dropped after the event
   *      (see boothScoreEffect above). This is the authoritative signal and
   *      it covers every nullification the API publishes a score change for.
   *
   *   2. The play text says so. Real ESPN/NFL gamebook wording, verified on
   *      published play-by-play:
   *        "…for 98 yards, TOUCHDOWN NULLIFIED by Penalty.PENALTY on
   *         PHI-J.Carter, Defensive Offside, 4 yards, enforced at PHI 36 -
   *         No Play."
   *        "…pass short left to M.Brown for 4 yards, TOUCHDOWN NULLIFIED by
   *         Penalty.PENALTY on KC-J.Smith-Schuster, Offensive Pass
   *         Interference, 10 yards, enforced at PHI 4 - No Play."
   *        "…for 28 yards, TOUCHDOWN.The Replay Official reviewed the runner
   *         broke the plane ruling, and the play was REVERSED.…to KC 1 for 27
   *         yards"
   *      so three text shapes are recognised, each of which must also name a
   *      score (touchdown / field goal / extra point / two-point / safety):
   *        a) "<score> NULLIFIED" (the official wording; punctuation around
   *           "by Penalty" / "PENALTY" varies between feeds, so it is not
   *           matched on),
   *        b) a "- No Play" penalty on a play that scored (an accepted foul
   *           wipes the score — NFL rule 10-5-5), and
   *        c) a REVERSED / OVERTURNED replay verdict on a play that scored.
   *
   * Wording that merely mentions a score without any of the three shapes
   * (e.g. an ordinary touchdown, or a flag on the ensuing kickoff) is NOT a
   * nullification and never reaches these feeds.
   */
  const SCORE_WORDS = /\btouchdown\b|\bfield goal\b|\bextra point\b|\btwo[- ]point\b|\b2[- ]point\b|\bconversion\b|\bsafety\b|\bPAT\b/i;

  function mentionsScore(text) {
    return SCORE_WORDS.test(String(text || ''));
  }

  /*
   * True when the play text itself reports a score being wiped out.
   * `text` is the play description; nothing else is inspected.
   */
  function nullifiedScoreText(text) {
    const t = String(text || '');
    if (!t) return false;
    if (!mentionsScore(t)) return false;
    // a) explicit "TOUCHDOWN NULLIFIED" / "FIELD GOAL NULLIFIED" wording.
    if (/\bnullified\b/i.test(t)) return true;
    // b) an accepted foul that wipes the down ("- No Play") on a scoring play.
    if (/\bno play\b/i.test(t)) return true;
    // c) a replay verdict that reversed / overturned a scoring play.
    if (/\breversed\b/i.test(t) || /\boverturned\b/i.test(t)) return true;
    return false;
  }

  /*
   * A booth event nullifies a score when ESPN's running score actually
   * dropped, or when the play text reports the nullification (above).
   */
  ...
  function boothEventContext(event, plays, index) {
    ...
    // The score named on the message: the nearest scoring play in the
    // removal-note window, used to caption a completed rollback.
